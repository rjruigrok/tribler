            ...
            if not message.source.startswith(u"circuit_"):
                ...
                for pex_peer in set(pex_peers):
                    ...
                    if cache.info_hash not in self.infohash_ip_circuits:
                        self.tunnel_logger.info("Create end-to-end on pex_peer %s" % repr(pex_peer_sock))
                        self.create_e2e(cache.circuit, pex_peer_sock, cache.info_hash, pex_peer_key)
